    ...
    public class FirebaseHelper
    {
        ...
        public async Task<List<LeaveManagement>> GetLeaves(string userId, string year, string month, string leaveType)
        {
            ...
            foreach (var childSnapshot in dataSnapshot)
            {
                var value = childSnapshot.Object;
                var valueJson = value.ToString();
                var Leave = JsonConvert.DeserializeObject<LeaveManagement>(valueJson);

                if (string.IsNullOrEmpty(leaveType) || Leave.TypeOfLeaves.Equals(leaveType, StringComparison.OrdinalIgnoreCase))
                {
                    var LeaveViewModel = new LeaveManagement
                    {
                        Date = Leave.Date,
                        FromTime = Leave.FromTime,
                        ToTime = Leave.ToTime,
                        TypeOfLeaves = Leave.TypeOfLeaves,
                        Status = Leave.Status
                    };
